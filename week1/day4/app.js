...
let thisSize = thisArray.length - 1;
let thatSize = thatArray.length - 1;

let randomThisIndex = Math.floor(Math.random() * thisSize);
let randomThatIndex = Math.floor(Math.random() * thatSize);

function startUpIdea(){
    console.log(`Your startup will be doing ${thisArray[randomThisIndex]} marketed to the ${thatArray[randomThatIndex]}`);
}
